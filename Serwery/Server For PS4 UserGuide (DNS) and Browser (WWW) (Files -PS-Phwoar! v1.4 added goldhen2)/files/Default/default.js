...
function displayHome() {
  $(document).prop("title", "Category Selection | Exploit Host by Al Azif / MOD By Leeful"),
...
function displayCategory(a) {
  $(document).prop("title", "Exploit Host by Al Azif / MOD By Leeful"),
  window.history.pushState({
    location: a,
    modal: !1
  }, null, "#".concat(a)),
  $("#title").text("Select Host Menu To Install:"),
  $("#header").text(""),
...
function triggerAutoload(a, b, c) {
  ...
  if (d) {
    var e = d.split("/")[0],
    f = d.split("/")[1];
    navigator.onLine || window.Menu[e].offline && window.Menu[e].entries[f].offline ? navigator.onLine && !c && window.Menu[e].offline && window.Menu[e].entries[f].offline ? cacheEntry(e, f) : exploitLoader(e, f) : exploitLoader(e, f) //leeful
  }
}
...
function cacheInterface(a) {
  ...
  if ("ondownloading" === a)
	...
	triggerAutoload(); location.reload();
  }
}
